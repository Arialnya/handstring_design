...
public class MonthlyStatisticsFragment extends Fragment {
    ...
    private void setupHeartRateChart() {
        ...
        xAxis.setValueFormatter(new ValueFormatter() {
            ...
            public String getFormattedValue(float value) {
                ...
                calendar.add(Calendar.DAY_OF_MONTH, (int) value - 29);
    ...
    private void loadHeartRateData() {
        ...
        // 计算每天的平均心率
        float[] dailyAverages = new float[30];
        ...
        for (HealthData data : dataList) {
            if (data.getHeartRate() > 0) {
                ...
                if (daysAgo >= 0 && daysAgo < 30) {
                    dailyAverages[29 - daysAgo] += data.getHeartRate();
    ...
    private void loadSleepData() {
        ...
        float[] deepSleep = new float[30];
        float[] lightSleep = new float[30];
        float[] awake = new float[30];
        ...
        for (HealthData data : dataList) {
            if (data.getDeepSleepMinutes() > 0 || data.getLightSleepMinutes() > 0 || data.getAwakeMinutes() > 0) {
                ...
                int daysAgo = (int) ((endDate.getTime() - data.getTimestamp().getTime()) / (1000 * 60 * 60 * 24));
                if (daysAgo >= 0 && daysAgo < 30) {
                    deepSleep[29 - daysAgo] = data.getDeepSleepMinutes();
                    lightSleep[29 - daysAgo] = data.getLightSleepMinutes();
                    awake[29 - daysAgo] = data.getAwakeMinutes();
                }
            }
        }
